...
void init_gpux_fft() {
   ...
   tmpPtr = alloc_double_tensor_device(chunkNx_fft, Ny, Nz);
   pitchX = tmpPtr.pitch;
   ...
   for (cnti = 0; cnti < Ngpu; cnti ++) {
      ...
      for (cntj = 0; cntj < chunksNx_fft; cntj ++) {
         ...
         gpux_fft[cnti].psix_h2d[cntj].srcPtr = make_cudaPitchedPtr(psi[cpuNx_fft + d_offsetNx + offsetNx][0], Nz * sizeof(double), Nz, Ny);
         gpux_fft[cnti].psix_h2d[cntj].dstPtr = gpux_fft[cnti].psix[cntj];
         gpux_fft[cnti].psix_h2d[cntj].extent = make_cudaExtent(Nz * sizeof(double), Ny, chunkNx_fft);
         gpux_fft[cnti].psix_h2d[cntj].kind = cudaMemcpyHostToDevice;

         gpux_fft[cnti].psix_d2h[cntj].srcPtr = gpux_fft[cnti].psix[cntj];
         gpux_fft[cnti].psix_d2h[cntj].dstPtr = make_cudaPitchedPtr(psi[cpuNx_fft + d_offsetNx + offsetNx][0], Nz * sizeof(double), Nz, Ny);
         gpux_fft[cnti].psix_d2h[cntj].extent = make_cudaExtent(Nz * sizeof(double), Ny, chunkNx_fft);
...
void init_gpux_nyz() {
   ...
   // cbetax  (chunkNx, Ny, Nz)

   tmpPtr = alloc_double_tensor_device(chunkNx_nyz, Ny, Nz);
   pitchX = tmpPtr.pitch;
   ...
   for (cnti = 0; cnti < Ngpu; cnti ++) {
      ...
      gpux_nyz[cnti].calphay = alloc_double_vector_device(Ny - 1);
      gpux_nyz[cnti].calphaz = alloc_double_vector_device(Nz - 1);
      gpux_nyz[cnti].cgammay = alloc_double_vector_device(Ny - 1);
      gpux_nyz[cnti].cgammaz = alloc_double_vector_device(Nz - 1);
      ...
      cudaCheckError(cudaMemcpy(gpux_nyz[cnti].calphay, calphay, (Ny - 1) * sizeof(double), cudaMemcpyHostToDevice));
      cudaCheckError(cudaMemcpy(gpux_nyz[cnti].calphaz, calphaz, (Nz - 1) * sizeof(double), cudaMemcpyHostToDevice));
      cudaCheckError(cudaMemcpy(gpux_nyz[cnti].cgammay, cgammay, (Ny - 1) * sizeof(double), cudaMemcpyHostToDevice));
      cudaCheckError(cudaMemcpy(gpux_nyz[cnti].cgammaz, cgammaz, (Nz - 1) * sizeof(double), cudaMemcpyHostToDevice));
      ...
      for (cntj = 0; cntj < chunksNx_nyz; cntj ++) {
         ...
         gpux_nyz[cnti].psix_h2d[cntj].srcPtr = make_cudaPitchedPtr(psi[cpuNx_nyz + d_offsetNx + offsetNx][0], Nz * sizeof(double), Nz, Ny);
         gpux_nyz[cnti].psix_h2d[cntj].dstPtr = gpux_nyz[cnti].psix[cntj];
         gpux_nyz[cnti].psix_h2d[cntj].extent = make_cudaExtent(Nz * sizeof(double), Ny, chunkNx_nyz);
         gpux_nyz[cnti].psix_h2d[cntj].kind = cudaMemcpyHostToDevice;

         gpux_nyz[cnti].psix_d2h[cntj].srcPtr = gpux_nyz[cnti].psix[cntj];
         gpux_nyz[cnti].psix_d2h[cntj].dstPtr = make_cudaPitchedPtr(psi[cpuNx_nyz + d_offsetNx + offsetNx][0], Nz * sizeof(double), Nz, Ny);
         gpux_nyz[cnti].psix_d2h[cntj].extent = make_cudaExtent(Nz * sizeof(double), Ny, chunkNx_nyz);
...
void init_gpuy_lux() {
   ...
   // psiy    (Nx, chunkNy_lux, Nz)
   // cbetay  (Nx, chunkNy_lux, Nz)

   tmpPtr = alloc_double_tensor_device(Nx, chunkNy_lux, Nz);
   pitchY = tmpPtr.pitch;
   ...
   for (cnti = 0; cnti < Ngpu; cnti ++) {
      ...
      gpuy_lux[cnti].calphax = alloc_double_vector_device(Nx - 1);
      gpuy_lux[cnti].cgammax = alloc_double_vector_device(Nx - 1);
      ...
      cudaCheckError(cudaMemcpy(gpuy_lux[cnti].calphax, calphax, (Nx - 1) * sizeof(double), cudaMemcpyHostToDevice));
      cudaCheckError(cudaMemcpy(gpuy_lux[cnti].cgammax, cgammax, (Nx - 1) * sizeof(double), cudaMemcpyHostToDevice));
      ...
      for (cntj = 0; cntj < chunksNy_lux; cntj ++) {
         ...
         gpuy_lux[cnti].psiy_h2d[cntj].srcPtr = make_cudaPitchedPtr(psi_t[0][0], Nz * sizeof(double), Nz, localNy);
         ...
         gpuy_lux[cnti].psiy_h2d[cntj].extent = make_cudaExtent(Nz * sizeof(double), chunkNy_lux, Nx);
         gpuy_lux[cnti].psiy_h2d[cntj].kind = cudaMemcpyHostToDevice;

         gpuy_lux[cnti].psiy_d2h[cntj].srcPtr = gpuy_lux[cnti].psiy[cntj];
         gpuy_lux[cnti].psiy_d2h[cntj].dstPtr = make_cudaPitchedPtr(psi_t[0][0], Nz * sizeof(double), Nz, localNy);
         gpuy_lux[cnti].psiy_d2h[cntj].dstPos = make_cudaPos(0, cpuNy_lux + d_offsetNy + offsetNy, 0);
         gpuy_lux[cnti].psiy_d2h[cntj].extent = make_cudaExtent(Nz * sizeof(double), chunkNy_lux, Nx);
...
void free_gpu() {
   ...
   for (cnti = 0; cnti < Ngpu; cnti ++) {
      ...
      free_double_vector_device(gpuy_lux[cnti].calphax);
      free_double_vector_device(gpux_nyz[cnti].calphay);
      free_double_vector_device(gpux_nyz[cnti].calphaz);
      free_double_vector_device(gpuy_lux[cnti].cgammax);
      free_double_vector_device(gpux_nyz[cnti].cgammay);
      free_double_vector_device(gpux_nyz[cnti].cgammaz);
...
void calcnorm_gpu_p2(double tmp) {
   int cnti, cntj;
   ...
   for (cnti = 0; cnti < Ngpu; cnti ++) {
      ...
      for (cntj = 0; cntj < chunksNx_fft; cntj ++) {
         calcnorm_kernel<<<gpux_fft[cnti].dimGrid3d_dd2, gpux_fft[cnti].dimBlock3d_dd2, 0, gpu[cnti].exec_stream[cntj]>>>(gpux_fft[cnti].psix[cntj], tmp);
...
void calcpsidd2_gpu_p1_1ce() {
   ...
   for (cnti = 0; cnti < Ngpu; cnti ++) {
      ...
      for (cntj = 0; cntj < chunksNx_fft; cntj ++) {
         cudaCheckError(cudaMemcpy3DAsync(&(gpux_fft[cnti].psix_d2h[cntj]), gpu[cnti].exec_stream[cntj]));
      }
